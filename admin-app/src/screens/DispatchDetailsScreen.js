...
export const DispatchDetailsScreen = ({ route, navigation }) => {
  ...
  // Admin can mark as received if they want to override, but usually staff does it
  const canConfirm = item.status !== "RECEIVED";
  ...
  const onConfirm = async () => {
    try {
      ...
      await refresh();
      Toast.show({ type: "success", text1: "Success", text2: "Shipment finalized manually by Admin." });
      navigation.goBack();
    } catch (e) {
